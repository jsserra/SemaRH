...
import com.pmm.semarh.dao.NacionalidadeDao;
 ...
 */
@Named
@Path("nacionalidadews")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class NacionalidadeWs {

    @EJB
    NacionalidadeDao daoNacionalidade;
    

    @GET
    @Path("getListaNacionalidade")
    public List<Nacionalidade> getListaNacionalidade(@Context HttpHeaders headers, @Context HttpServletRequest request) {
       ...
       //     daoUserLog.criarLog("Lista de Sexos", "Listar", headers.getRequestHeader("chave").get(0), request.getPathInfo(), headers.getRequestHeader("appv").get(0),"SDGC", request.getRemoteAddr());
            return daoNacionalidade.getListaNacionalidade();
