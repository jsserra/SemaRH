...
import com.pmm.semarh.dao.BairrosDao;
import com.pmm.semarh.dao.CidadesDao;
import com.pmm.semarh.dao.EstadosDao;
import com.pmm.semarh.model.Bairros;
import com.pmm.semarh.model.Cidades;
import com.pmm.semarh.model.Estados;
...
public class EnderecosWs {
    ...
    public List<Bairros> getListaBairros(@Context HttpHeaders headers, @Context HttpServletRequest request) {
        ...
        try {
            ...
            return daoBairros.getListaBairros();
    ...
    public List<Cidades> getListaCidades(@Context HttpHeaders headers, @Context HttpServletRequest request) {

        try {
            //    Integer idUserMenu = 1;
            //     daoUserLog.criarLog("Lista de Sexos", "Listar", headers.getRequestHeader("chave").get(0), request.getPathInfo(), headers.getRequestHeader("appv").get(0),"SDGC", request.getRemoteAddr());
            return daoCidades.getListaCidades();
        } catch (Exception exception) {
            throw new WebApplicationException(Response.status(Response.Status.BAD_REQUEST).entity(exception.getMessage()).build());
        }
    }
    ...
    public List<Estados> getListaEstados(@Context HttpHeaders headers, @Context HttpServletRequest request) {
        ...
        try {
            //    Integer idUserMenu = 1;
            //     daoUserLog.criarLog("Lista de Sexos", "Listar", headers.getRequestHeader("chave").get(0), request.getPathInfo(), headers.getRequestHeader("appv").get(0),"SDGC", request.getRemoteAddr());
            return daoEstados.getListaEstados();
        } catch (Exception exception) {
            throw new WebApplicationException(Response.status(Response.Status.BAD_REQUEST).entity(exception.getMessage()).build());
        }
    }
