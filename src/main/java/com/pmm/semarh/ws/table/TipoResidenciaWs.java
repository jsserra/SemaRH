...
import com.pmm.semarh.dao.TipoResidenciaDao;
import com.pmm.semarh.model.TipoResidencia;
 ...
 */
@Named
@Path("tiporesidenciaws")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class TipoResidenciaWs {

    @EJB
    TipoResidenciaDao daoTipoResidencia;
   

    @GET
    @Path("getListaTipoResidencia")
    public List<TipoResidencia> getListaTipoResidencia(@Context HttpHeaders headers, @Context HttpServletRequest request) {
       ...
       //     daoUserLog.criarLog("Lista de Sexos", "Listar", headers.getRequestHeader("chave").get(0), request.getPathInfo(), headers.getRequestHeader("appv").get(0),"SDGC", request.getRemoteAddr());
            return daoTipoResidencia.getListaTipoResidencia();
